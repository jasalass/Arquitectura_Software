  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const seccion = await tx.seccion.findUnique({
        ...
        select: { id: true, asignatura_id: true, periodo_id: true, cupos_totales: true, cupos_tomados: true }
      });
      ...
      if (requiredIds.length > 0) {
        ...
        const ok = await tx.inscripcion.findMany({
          where: {
            alumno_ref: alumnoRef,                 // <-- String @db.Uuid en tu schema
            estado: 'INSCRITA',
            seccion: {
        ...
        if (missingIds.length > 0) {
          ...
          const lista = faltantes.map(f => `${f.codigo} - ${f.nombre}`).join(', ');
...
app.get('/inscripcion/alumno-estado/:alumnoRef', async (req, res) => {
  const { alumnoRef } = req.params;
  try {
    const estado = await prisma.alumno_estado.findUnique({
...
app.patch('/inscripcion/alumno-estado/:alumnoRef', async (req, res) => {
  ...
  try {
    const actualizado = await prisma.alumno_estado.upsert({
      where: { alumno_ref: alumnoRef },
